...
void FormatPartitionFAT32(uint64_t part_sectors)
{
    ...
    {
        if (i % 32 == 0)
            kprintf("[Format] FAT1 Init: %d/128\n", i);
        NVMe::g_nvme->Write(fat1_start + i, buf, 1);
    }
    kprintf("[Format] FAT1 Init Complete.\n");

    // FAT2の残り
    for (uint32_t i = 1; i < 128; ++i)
    {
        if (i % 32 == 0)
            kprintf("[Format] FAT2 Init: %d/128\n", i);
        NVMe::g_nvme->Write(fat2_start + i, buf, 1);
    }
    kprintf("[Format] FAT2 Init Complete.\n");
    ...
    uint64_t data_start_lba =
        kPartitionStartLBA + reserved_sectors + (num_fats * fat_sz_sec);
    ...
    {
        kprintf("[Format] RootDir Init: %d/%d\n", i, sec_per_clus);
